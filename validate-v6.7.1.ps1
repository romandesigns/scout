...
if ($Version -ne "6.7.1") {
    throw "VERSION is '$Version', expected 6.7.1."
}
...
if ($Sw -notmatch 'const VERSION="6\.7\.1";') {
    throw "Service worker version mismatch."
}

Write-Host "Release identity: PASS" -ForegroundColor Green

$PageSource = Get-Content (Join-Path $PSScriptRoot "web\app\page.tsx") -Raw
foreach ($Marker in @(
    'TwentyFourHourPanel',
    'getTwentyFourHourStocks',
    'id:"24h"',
    'twentyFourHourResult'
)) {
    if ($PageSource -notmatch [regex]::Escape($Marker)) {
        throw "24H UI integration marker missing: $Marker"
    }
}
Write-Host "24H UI integration: PASS" -ForegroundColor Green
$MomentumMarkers = @('momentumProfile','EXTREME MOMENTUM','FRESH ENTRY',"DON'T CHASE",'POWER {momentum.score}')
foreach ($Marker in $MomentumMarkers) {
    if ($PageSource -notmatch [regex]::Escape($Marker)) { throw "Momentum UI marker missing: $Marker" }
}
Write-Host "Momentum-intensity UI: PASS" -ForegroundColor Green
...
Write-Host "SCOUT v6.7.1 LOCAL VALIDATION PASS" -ForegroundColor Green
