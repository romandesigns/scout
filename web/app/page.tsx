...
type WorkbenchProps = {
  ...
  catalysts: Catalyst[];
  gainers: Gainer[];
  twentyFourHour: TwentyFourHourStock[];
  halts: Halt[];
...
function decisionChart(finding?:Finding) {
  const chart=finding?.candidate_profile?.decision_chart;
  const aligned=finding?.candidate_profile?.multi_timeframe?.qualified===true;
  return {
    primary:(chart?.primary_seconds??(aligned?60:finding?.detection_timeframe_seconds??15)) as 15|30|60|300,
    trigger:chart?.trigger_seconds??30,
    context:chart?.context_seconds??300,
    instruction:chart?.instruction??(aligned?"Open 1m · confirm on 30s · 5m context aligned":null),
  };
}
...
function FindingRow({ finding, selected, onSelect, watching, onToggleWatch }: { finding: Finding; selected: boolean; onSelect: () => void; watching?: boolean; onToggleWatch?: () => void }) {
  ...
  const chart=decisionChart(finding);
...
function SettingsPanel({ connected, onNotifications, scanner, saveScanner, scannerBusy, scannerMessage, backendVersion }: { connected:boolean; onNotifications:()=>void; scanner:ScannerSettings; saveScanner:(value:ScannerSettings)=>Promise<void>; scannerBusy:boolean; scannerMessage:string; backendVersion?:string }) {
  ...
  return <div className="flex h-full min-h-0 flex-col"><PanelTitle icon={<IconSettings size={14}/>} title="SETTINGS"/><div className="space-y-2 p-3">
    ...
    <div className="scanner-range-card"><div className="scanner-range-title"><div><b>Scout Trader</b><span>Alpaca paper execution · confirmed A-rank clean momentum only</span></div><Badge data-tone={trader?.enabled?"green":trader?.configured?"gray":"orange"}>{trader?.enabled?"PAPER ACTIVE":trader?.configured?"OFF":"NOT CONFIGURED"}</Badge></div><div className="settings-row"><div><span>Automatic paper trades</span><div className="text-[10px] scout-muted">${trader?.position_notional??100} bracket · up to {trader?.max_positions??100} concurrent evaluations · ${trader?.daily_loss_limit??25} daily loss guard</div></div><Switch checked={Boolean(trader?.enabled)} disabled={!trader?.configured||traderBusy} onCheckedChange={enabled=>void updateTrader({enabled})}/></div><div className="scanner-range-inputs"><label>Risk<Input type="number" value={1} disabled/></label><label>Reward<Input type="number" min="1" max="10" step="0.25" value={trader?.risk_reward??3} disabled={!trader||traderBusy} onChange={e=>setTrader(current=>current?{...current,risk_reward:Number(e.target.value)}:current)}/></label></div><div className="scanner-range-inputs"><label>Trade size ($)<Input type="number" min="10" max="10000" step="10" value={trader?.position_notional??100} disabled={!trader||traderBusy} onChange={e=>setTrader(current=>current?{...current,position_notional:Number(e.target.value)}:current)}/></label><label>Evaluation capacity<Input type="number" min="1" max="100" step="1" value={trader?.max_positions??100} disabled={!trader||traderBusy} onChange={e=>setTrader(current=>current?{...current,max_positions:Number(e.target.value)}:current)}/></label></div><Button className="w-full" disabled={!trader?.configured||traderBusy||!trader||trader.risk_reward<1||trader.risk_reward>10||trader.max_positions<1||trader.max_positions>100} onClick={()=>void updateTrader({risk_reward:trader?.risk_reward??3,position_notional:trader?.position_notional??100,max_positions:trader?.max_positions??100})}>{traderBusy?"Applying…":"Apply trader evaluation settings"}</Button><div className="notice-box">Paper only. Each confirmed clean A episode is sampled once. Stops are structural and capped at {trader?.max_stop_pct??3}%; capacity and the daily-loss guard bound aggregate exposure.</div>{trader?.performance&&<div className="text-[10px] scout-muted">Trades {trader.performance.total} · Open {trader.performance.open}/{trader.max_positions} · Closed {trader.performance.closed} · P&amp;L ${trader.performance.realized_pl.toFixed(2)}</div>}{traderMessage&&<div className="notice-box">{traderMessage}</div>}{trader?.last_error&&<div className="notice-box">Last trader error: {trader.last_error}</div>}</div>
...
function ChartGroup({ finding, allFindings, onSelectFinding, onClose, onMaximize, maximized, active, pinned, onTogglePin, onActivate, pollOffsetMs, refreshNonce, tabLimit }: {
  ...
  useEffect(()=>{const chart=decisionChart(finding);if(chart.instruction||readUiPreferences().openDetectionTimeframe)setTimeframe(chart.primary);},[finding?.id]);
  ...
  return <section className="chart-group" data-active={active||undefined} onPointerDown={onActivate}>
    ...
    <div className="chart-header">
      ...
      <div className="flex items-center gap-1">{decisionChart(finding).instruction&&<span className="chart-mode-label">PRIMARY</span>}<Select className="timeframe-select" label="Chart timeframe" value={String(timeframe)} onValueChange={value=>setTimeframe(Number(value) as 15|30|60|300)} options={[{value:"15",label:"15s"},{value:"30",label:"30s"},{value:"60",label:"1m"},{value:"300",label:"5m"}]}/><span className="chart-mode-label">{frozen?"DETECTION":"LIVE"}</span><IconButton label="Chart menu"><IconDots size={14}/></IconButton></div>
    </div>
...
function GateFeedbackSection({ finding, verification, onVerification }: { finding:Finding; verification:FindingVerification|null; onVerification:(v:FindingVerification)=>void }) {
  const [busy,setBusy]=useState<"accurate"|"inaccurate"|null>(null);
  const gate=finding.candidate_profile?.imminent_move_gate;
  if(!gate||gate.status!=="scored")return null;
  const current=verification?.review?.gate_feedback;
  async function mark(value:"accurate"|"inaccurate"){
    if(!API_CONFIGURED)return;
    setBusy(value);
    try{onVerification(await saveGateFeedback(finding.id,current===value?null:value));}
    catch{/* best-effort; UI simply keeps the prior state on failure */}
    finally{setBusy(null);}
  }
  return <InspectorSection title="IMMINENT MOVE GATE (SHADOW)">
    <KV k="Probability" v={`${(gate.probability!==undefined?gate.probability*100:0).toFixed(1)}%`}/>
    <KV k="Threshold" v={gate.threshold!==undefined?`${(gate.threshold*100).toFixed(1)}%`:"—"}/>
    <KV k="Reading" v={gate.would_pass?"WOULD PASS":"WOULD REJECT"}/>
    <div className="notice-box">Shadow-only research signal — does not affect this alert's delivery. Was this reading accurate?</div>
    <div className="mt-2 flex gap-2">
      <Button variant={current==="accurate"?"default":"ghost"} disabled={busy!==null} onClick={()=>mark("accurate")}>{busy==="accurate"?"Saving…":"Mark accurate"}</Button>
      <Button variant={current==="inaccurate"?"default":"ghost"} disabled={busy!==null} onClick={()=>mark("inaccurate")}>{busy==="inaccurate"?"Saving…":"Mark inaccurate"}</Button>
    </div>
  </InspectorSection>;
}
...
function Inspector({ finding, onNotifications }: { finding?:Finding; onNotifications:()=>void }) {
  ...
  return <div className="h-full overflow-y-auto p-3">
    ...
    {tab==="overview"&&<>
    {decisionChart(finding).instruction&&<InspectorSection title="CHART PLAN"><KV k="Open first" v="1-minute chart"/><KV k="Confirm trigger" v="30-second chart"/><KV k="Structure context" v="5-minute chart"/><div className="notice-box">Fast tape (5–15s) is a timing/veto input, not the decision chart.</div></InspectorSection>}
...
function ActivityRail({ active, onActive, counts }: { active:ActivityView; onActive:(v:ActivityView)=>void; counts:Record<Exclude<ActivityView,"settings">,number> }) {
  const items: {id:Exclude<ActivityView,"settings">;label:string;icon:React.ReactNode;tone?:string}[]=[
    {id:"radar",label:"Radar",icon:<IconBolt size={18}/>,tone:"green"},{id:"24h",label:"24H Stocks",icon:<IconHistory size={17}/>,tone:"blue"},
...
export default function ScoutPage() {
  ...
  const [twentyFourHour,setTwentyFourHour]=useState<TwentyFourHourStock[]>([]);
  ...
  const refresh=useCallback(async(heavy=true)=>{
    ...
    const job=(async()=>{
      const results = await Promise.allSettled([
        getStatus(),getFindings(180),getCatalysts(80),getHalts(),getScannerSettings(),getAttention(80),getTwentyFourHourStocks(200),
      ]);
      const [statusResult,findingsResult,catalystsResult,haltsResult,scannerResult,attentionResult,twentyFourHourResult]=results;
      ...
      if(twentyFourHourResult.status==='fulfilled')setTwentyFourHour(twentyFourHourResult.value);
      if(heavy){
