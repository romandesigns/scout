...
def test_list_findings_can_query_mature_actionable_rows_directly(tmp_path):
    ...
    with store.lock:
        ...
        store.db.execute(
            """INSERT INTO findings(ticker,stage,detected_at,price,score,evidence_json,signals_json,
               quality_label,quality_score,actionable_rank,rejection_reasons_json,hybrid_sources_json,
               recipe_present_json,recipe_missing_json,candidate_profile_json,shadow_mode)
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            ("SHADOW_A","EARLY",1100.0,5.0,1.0,"[]","[]","CLEAN",100,"A","[]","[]","[]","[]","{}",1),
        )
        store.db.execute(
            """INSERT INTO findings(ticker,stage,detected_at,price,score,evidence_json,signals_json,
               quality_label,quality_score,actionable_rank,rejection_reasons_json,hybrid_sources_json,
               recipe_present_json,recipe_missing_json,candidate_profile_json,timeliness_label)
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            ("LATE_A","EARLY",1200.0,5.0,1.0,"[]","[]","CLEAN",100,"A","[]","[]","[]","[]","{}","LATE"),
        )
        store.db.commit()
